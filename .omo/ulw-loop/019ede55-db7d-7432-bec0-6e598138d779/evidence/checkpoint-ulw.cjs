...
const codexGoal = JSON.stringify({ objective, status: 'active' });
...
const qualityGate = JSON.stringify({
  codeReview: { by: 'parent', recommendation: 'APPROVE', codeQualityStatus: 'CLEAR', evidence: 'Diff reviewed; TypeScript files are 15 and 14 pure LOC; no sample behavior strings remain.', blockers: [] },
  manualQa: {
    by: 'parent',
    status: 'passed',
    ...
    artifactRefs: [
      { id: 'npm-run-build', kind: 'cli-transcript', path: `.omo/ulw-loop/${session}/evidence/npm-run-build.txt` },
      { id: 'obsidian-runtime-smoke', kind: 'cli-transcript', path: `.omo/ulw-loop/${session}/evidence/obsidian-runtime-smoke.txt` },
      { id: 'obsolete-sample-scan', kind: 'log', path: `.omo/ulw-loop/${session}/evidence/obsolete-sample-scan.txt` },
    ],
  },
  gateReview: { by: 'parent', recommendation: 'APPROVE', evidence: 'Requested files updated and verification artifacts are non-empty.', blockers: [] },
  iteration: { fullRerun: true, status: 'passed', rerunCommands: ['npm.cmd run build', 'npm.cmd run lint'], evidence: 'Both commands exited 0.' },
  criteriaCoverage: {
    totalCriteria: 51,
    passCount: 51,
...
let count = 0;
for (let index = 0; index < plan.goals.length; index += 1) {
  const goal = plan.goals[index];
  const args = [cli, 'ulw-loop', 'checkpoint', '--goal-id', goal.id, '--status', 'complete', '--evidence', evidence, '--codex-goal-json', codexGoal, '--json'];
  if (index === plan.goals.length - 1) {
    args.splice(args.length - 1, 0, '--quality-gate-json', qualityGate);
  }
  const result = spawnSync(process.execPath, args, { encoding: 'utf8' });
  if (result.status !== 0) {
    console.error(result.stdout);
    console.error(result.stderr);
    throw new Error(`checkpoint failed for ${goal.id}`);
  }
  count += 1;
}
console.log(`checkpointed goals: ${count}`);
